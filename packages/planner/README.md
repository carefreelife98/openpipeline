...
- **The catalog is loaded at most once per `plan()` call** (cached across `select` re-entries) and its `cleanup()` runs exactly once, on every exit path (success, failure, or abort).
