...
<!-- Which package(s) / area: core, nodes, planner, runtime, mcp, store-*, server, react, examples, ci, docs -->

## Checklist

- [ ] `pnpm build && pnpm typecheck` — 9/9 (build before typecheck)
- [ ] `pnpm lint && pnpm format:check` — clean
- [ ] `pnpm test:ci` — passing, coverage thresholds met
- [ ] Example smokes still green if touched (`quickstart`/`planner-quickstart`/`mcp`/`prisma`/`server`; `playground` builds)
